...
def prepare_dataset(
    ...
    hf_dataset = load_dataset(dataset_name, split=split, keep_in_memory=False)

    if max_samples is not None:
        hf_dataset = hf_dataset.select(range(min(max_samples, len(hf_dataset))))

    print(f"Processing {len(hf_dataset)} samples...")
    print(f"Batch size: {batch_size} (GPU accelerated)")
...
def main():
    if DATASET_INFO_PATH := os.getenv("DATASET_INFO_PATH"):
        ...
        print("No dataset information path provided. Preparing dataset...")
        prepare_dataset()
